...
setInterval(() => { //Spam Filter
    for (let i = 0; i < Object.keys(userMsgs).length; i++) {
        if (userMsgs[Object.keys(userMsgs)[i]] > CONFIG.MAXMSGPERSEC) {
            client.chatMessage(Object.keys(userMsgs)[i], "Sorry but we do not like spamming. You've been removed and blocked!");
            client.removeFriend(Object.keys(userMsgs)[i]);
            client.blockUser(Object.keys(userMsgs)[i]);
            for (let j = 0; j < CONFIG.Owner.length; j++) {
                client.chatMessage(CONFIG.Owner[j], "Steam #" + Object.keys(userMsgs)[i] + " has been blocked for spamming");
            }
        }
    }
    userMsgs = {};
...
client.on("webSession", (sessionID, cookies) => {
    ...
    manager.getInventoryContents(753, 6, true, (ERR, INV, CURR) => {
        if (ERR) {
            ...
            let playThis = +My_gems + " Gems > Buy/Sell Gems (!prices)";
...
client.on("friendRelationship", (SENDER, REL) => {
    community.getSteamUser(SENDER, function(err, user) {
        if (err) {
            return console.log("[" + getTime() + "] Error checking current friend relationship with new customer : " + err);
        } else {
            if (REL === 2) {
                console.log("[" + getTime() + "] " + "[New Friend] - " + user.name + " > " + SENDER.getSteamID64() + " - SteamID");
...
manager.on('newOffer', offer => {
    offer.getUserDetails((err, me, them) => {
        ...
        console.log("[" + getTime() + "] " + "[New Trade Offer] From:  " + offer.partner.getSteamID64());
...
function ProccessTradeOffer(offer) {
    ...
    ///////////////
    offer.getUserDetails((err, me, them) => {
        if (err) return console.log("[" + getTime() + "] An error occured while processing a trade : " + err);
        //////////////////
        if (CONFIG.Owner.indexOf(PartnerID) >= 0) {
            offer.accept((err, status) => {
                if (err) {
                    console.log("[" + getTime() + "] Error occured while auto accepting admin trades : " + err);
                } else {
            ...
            offer.accept((err, status) => {
                if (err) {
                    console.log("[" + getTime() + "] Error occured accepting donations : " + err);
                } else {
            ...
            if (tag.includes("Profile Background") || tag.includes("Emoticon")) { // BGS/EMOTES 4 Gems Trade
                ...
                offer.decline((err) => {
                    if (err) console.log("[" + getTime() + "] Error declining the trade offer : " + err);
                    console.log("[" + getTime() + "] " + "[Declined Offer]  | " + PartnerID);
                });
            }
        } else if (CONFIG.Ignore_Msgs.indexOf(PartnerID) >= 0) { //*Checks that the person you're interacting with is/isn't on your ignore list.*//
        } else {
            offer.decline((err) => {
                if (err) console.log("[" + getTime() + "] Error declining the trade offer : " + err);
                console.log("[" + getTime() + "] " + "[Declined Offer] | " + PartnerID);
...
client.on("friendMessage", (SENDER, MSG) => {
    if (CONFIG.Ignore_Msgs.indexOf(SENDER.getSteamID64()) >= 0) {
        ...
        community.getSteamUser(SENDER, function(err, user) {
            if (err) {
                return console.log("[" + getTime() + "] Failure parsing users Steam Info. Possibly illegal ASCII letters in name OR steam failed to : " + err);
            }
            console.log("[" + getTime() + "] " + "[Incoming Chat Message] " + user.name + " > " + SENDER.getSteamID64() + " : " + MSG);
            ...
            if (MSG.toUpperCase() === "!HELP") {
				...
				client.chatMessage(SENDER, "Coded By: https://steamcommunity.com/id/mfwBan/");
                ...
                let n = MSG.toUpperCase().replace("!SELLCS ", ""),
                    Amount_of_Gems = parseInt(n) * CONFIG.Rates.SELL.CSGO_To_Gems,
                    TheirKeys = [];
                        ...
                        t.getUserDetails((ERR, ME, THEM) => {
                            if (ERR) {
                                ...
                                manager.getInventoryContents(753, 6, true, (ERR, MyInv, CURR) => {
                                    if (err) {
                                        client.chatMessage(SENDER, "Inventory refresh in session. Try again shortly please.");
                                                ...
                                                if (ERR2) {
                                                    ...
                                                    if (TheirKeys.length != n) {
                                                        if (TheirKeys.length > 0) {
                                                            /*error*/
                                                            client.chatMessage(SENDER, "You don't have enough CS:GO keys to make this trade: " + TheirKeys.length + " / " + n + "\r\n Tip: Try using !SellCS " + TheirKeys.length);
                                                        } else {
                                                            /*error*/
                                                            client.chatMessage(SENDER, "You don't have enough CS:GO keys to make this trade: " + TheirKeys.length + " / " + n);
                                                        }
                                ...
                                client.chatMessage(SENDER, "Make sure you do not have any Trade Holds.");
                            }
                        });
                    } else {
                    ...
                    if (n <= CONFIG.Rates.Key_Swaps.Max_Swap) {
                        ...
                        t.getUserDetails((ERR, ME, THEM) => {
                            if (ERR) {
                                ...
                                manager.getUserInventoryContents(SENDER.getSteamID64(), 440, 2, true, (ERR, INV, CURR) => {
                                    if (ERR) {
                                        console.log("## An unforseen error occurred while getting inventory: " + ERR);
                                        ...
                                        if (TheirKeys.length != n) {
                                            client.chatMessage(SENDER, "You don't have enough TF2 Keys: " + TheirKeys.length + " / " + n + " or they're not Tradeable Yet");
                                                ...
                                                if (ERR2) {
                                                    ...
                                                    if (MyKeys.length != My_CSGO) {
                                                        ...
                                                        t.send((ERR, STATUS) => {
                                                            if (ERR) {
                                                                client.chatMessage(SENDER, "Inventory refresh in session. Try again shortly please.");
                                                                console.log("## An unforseen error occurred while sending trade: " + ERR);
                                                            } else {
                            ...
                            if (ERR) {
                                ...
                                manager.getUserInventoryContents(SENDER.getSteamID64(), 730, 2, true, (ERR, INV, CURR) => {
                                    if (ERR) {
                                        ...
                                        if (TheirKeys.length != n) {
                                            client.chatMessage(SENDER, "You don't have enough CS:GO Keys: " + TheirKeys.length + " / " + n + " or they're not Tradeable Yet");
                ...
                let n = MSG.toUpperCase().replace("!BUYTF ", ""),
                    Amount_of_Gems = parseInt(n) * CONFIG.Rates.BUY.Gems_To_TF2_Rate,
                    MyKeys = [];
                if (!isNaN(n) && parseInt(n) > 0) {
                                            ...
                                            manager.getInventoryContents(440, 2, true, (ERR2, MyInv) => {
                                                if (ERR2) {
                                                    ...
                                                    if (MyKeys.length != n) {
                                                        if (MyKeys.length > 0) {
                                                            /*error*/
                                                            client.chatMessage(SENDER, "Sorry, I don't have enough TF2 keys to make this trade: " + MyKeys.length + " / " + n + "\r\nTip: Try using !BuyTF " + MyKeys.length);
                                                        } else {
                                                            /*error*/
                                                            client.chatMessage(SENDER, "Sorry, I don't have enough TF2 keys to make this trade: " + MyKeys.length + " / " + n + ", I'll restock soon!");
                                                        }
                                                    } else {
                                                        t.addMyItems(MyKeys);
                ...
                let n = MSG.toUpperCase().replace("!BUYCS ", ""),
                    Amount_of_Gems = parseInt(n) * CONFIG.Rates.BUY.Gems_To_CSGO_Rate,
                    MyKeys = [];
                if (!isNaN(n) && parseInt(n) > 0) {
                    if (n <= CONFIG.Restrictions.MaxBuy) {
                        let t = manager.createOffer(SENDER.getSteamID64());
                                            ...
                                            manager.getInventoryContents(730, 2, true, (ERR2, MyInv) => {
                                                if (ERR2) {
                                                    ...
                                                    if (MyKeys.length != n) {
                                                        if (MyKeys.length > 0) {
                                                            /*error*/
                                                            client.chatMessage(SENDER, "Sorry, I don't have enough CS:GO keys to make this trade: " + MyKeys.length + " / " + n + "\r\nTip: Try using !BuyCS " + MyKeys.length);
                                                        } else {
                                                            /*error*/
                                                            client.chatMessage(SENDER, "Sorry, I don't have enough CS:GO keys to make this trade: " + MyKeys.length + " / " + n + ", I'll restock soon!");
                                                        }
                                                    } else {
                                                        t.addMyItems(MyKeys);
                ...
                let n = MSG.toUpperCase().replace("!SELLTF ", ""),
                    Amount_of_Gems = parseInt(n) * CONFIG.Rates.SELL.TF2_To_Gems,
                    TheirKeys = [];
                if (!isNaN(n) && parseInt(n) > 0) {
                    if (n <= CONFIG.Restrictions.MaxSell) {
                        let t = manager.createOffer(SENDER.getSteamID64());
                ...
                if (MSG.toUpperCase().indexOf("!BLOCK") >= 0) { // !block
                    ...
                    let total_Bought = Bought.TF2[1] + Bought.CSGO[1] + Bought.CRAP[1], // weekly total profits
                        total_Sold = Sold.TF2[1] + Sold.CSGO[1] + Sold.CRAP[1], // weekly total sets sold
                        total_Bought2 = Bought.TF2[0] + Bought.CSGO[0] + Bought.CRAP[0], // lifetime total profits
                        total_Sold2 = Sold.TF2[0] + Sold.CSGO[0] + Sold.CRAP[0]; // lifetime total sold
...
manager.on("sentOfferChanged", (OFFER, OLDSTATE) => {
    ...
    if (OFFER.state == 3) {
        ...
        if (TradeType.includes("!BuyTF")) {
            client.chatMessage(OFFER.partner, "Trade Complete! Enjoy your Keys and please +rep my profile so others knows I work :) Have a nice day!");
			...
			Comment_User(OFFER.partner);
            client.chatMessage(OFFER.partner, "Trade Complete! Enjoy your Gems and please +rep my profile so others knows I work :) Have a nice day!");
			...
			Comment_User(OFFER.partner);
            client.chatMessage(OFFER.partner, "Trade Complete! Enjoy your TF2 Keys and please +rep my profile so others knows I work :) Have a nice day!");
			...
			Comment_User(OFFER.partner);
            client.chatMessage(OFFER.partner, "Trade Complete! Enjoy your CSGO and please +rep my profile so others knows I work :) Have a nice day!");
...
function Comment_User(SteamID){
	community.getSteamUser(SteamID, (ERR, USER) => {
		if (ERR) {
			console.log("## An error occurred while getting user profile: Usually private. " + ERR);
		} 
		else {
			USER.comment(CONFIG.Comment_After_Trade, (ERR) => {
			if (ERR) {
				console.log("## An error occurred while commenting on user profile: comments disabled for any reason. " + ERR);
			}
			});
		}
	});
    ...
    if (offer.itemsToGive.length == My_Bg_And_Emote) {
        ...
        if (typeof TheirGems[0] === 'undefined') {
            ...
            if (gem.amount == Price_In_Gems) {
                ...
                require('fs').writeFileSync('./SETTINGS/TotalSold.json', JSON.stringify(Database, undefined, "\t"));
                offer.accept((err, status) => {
                client.chatMessage(PartnerID, "Trade Complete! Enjoy and please +Rep my profile to let others know I work!");
                    if (err) {
                        console.log("[" + getTime() + "] Error accepting trade during selling your BG's Emotes : " + err);
                    }
					RefreshInventory();
					client.chatMessage(CONFIG.Owner[1], "[" + getTime() + "] Trade Accepted From : " + PartnerID + " - They bought your BGs/Emotes");
					console.log("[" + getTime() + "] Trade Accepted From : " + PartnerID + " - Bought your BGs/Emotes");
					Comment_User(offer.partner);
                });
            } else { // Not enough gems,decline
			    client.chatMessage(PartnerID, "Rates are incorrect. Please retry using the correct rates.");
                offer.decline((err) => {
                    if (err) {
		...
		client.chatMessage(PartnerID, "Sorry, Cloudbank is not for sale. Try again please with other items!");
        offer.decline((err) => {
            console.log("[" + getTime() + "] " + "[SellBG] Declined! " + PartnerID + " - They tried to buy something blacklisted!");
            if (err) {
				console.log("[" + getTime() + "] " + err);
				}
        });
...
function Buy_Bgs_And_Emotes(offer) {
    ...
    if (offer.itemsToGive.length == 1) {
        ...
        if (typeof MyGems[0] === 'undefined') {
            offer.decline((err) => {
                if (err) {
					console.log("[" + getTime() + "] Error declining trade , Likely steam : " + err);
				}
            });
        } else {
            let gem = MyGems[0];
            if (gem.amount == Price_In_Gems) {
                ...
                client.chatMessage(PartnerID, "Trade Complete! Enjoy and please +Rep my profile to let others know I work!");
                    ...
                    if (err) {
                        console.log("[" + getTime() + "] Error accepting trade while buying their bgs/emotes : " + err);
                    }
					RefreshInventory();
					client.chatMessage(CONFIG.Owner[1], "[" + getTime() + "] Trade Accepted From : " + PartnerID + " - They sold you BGs/Emotes");
					console.log("[" + getTime() + "] Trade Accepted From : " + PartnerID + " - Sold you BGs/Emotes");
					Comment_User(offer.partner);
                });
            } else {
				client.chatMessage(PartnerID, "You're trying to take something that aren't gems alone / The rates may be incorrect. Please try again.");
                offer.decline((err) => {
                    if (err) {
		...
		client.chatMessage(PartnerID, "Trade Validation Failed. You can only take Gems for your BGs/Emotes.");
        offer.decline((err) => {
            console.log("[" + getTime() + "] " + "[BuyBG] Declined! - " + PartnerID + " : Tried to take something for BGs/Emotes that arent Gems");
            if (err) {
				console.log("[" + getTime() + "] " + err);
			}
        });
